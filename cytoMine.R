...
"
Usage:
cytoMine (--inputDir=<files>  --outputDir=<results_dir> --mode=<mode>) [--transformMethod=<methodName> --barcodeScheme=<barcodeScheme> --beadMasses=<beads>  --FlowSom_k=<ClusterN> --downSample=<number> --perplexity=<p>  --theta=<theta> --max_iter=<max_iter> --num_threads=<num_threads> --sampleFile=<csvfile> --markerFile=<csvfile> --reclustFSFile=<csvfile> --reclustCXFile=<csvfile> --device=<plotType> --cellSelectionFile=<Cellselection> --ncols=<number> --nrows=<number>]
...
--cellSelectionFile=<csvfile>  Any filename in inputDir. This file contains the cells selected in a previous run for t-sne. There is no default.
--ncols=<number>              Number of columns in t-sne plots
--nrows=<number>              Number of rows in t-sne plots
" -> doc
...
## Cell selection file
if(!is.null(opts$cellSelectionFile))
{
  cellSelectionFile <- as.character(opts$cellSelectionFile)
} else
  cellSelectionFile<-opts$cellSelectionFile
...
## Facet per Group
df<-group_downsample(dr,downsample)
...
#        guides(color = guide_legend(override.aes = list(size = 4))),width=10,height = 10)
    ...
    } else if(!is.null(reclustCXFile))
  {
      ...
      local({
        ...
        ggp <- ggplot(dr,  aes(x = tSNE1, y = tSNE2, color = cell_mclust_CX)) +
          geom_point(size = 0.2) +
          theme_bw() +
          scale_color_manual(values = colp) +
          guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))
        message("CytoMine: Generating t-SNE plot for ClusterX clustering.")
        ...
        ## Facet per Group
        df<-group_downsample(dr,downsample)
        ggp <- ggplot(df,  aes(x = tSNE1, y = tSNE2, color = cell_mclust_CX)) +
          geom_point(size = 0.2) +
          theme_bw() +
          scale_color_manual(values = colp) +
          guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))
        ...
        ggsave(
          ...
          ggp + facet_wrap(~ condition,ncol=ncols,nrow=nrows),width=10,height = 10)
                            ...
                            mm <- match(ggdf$sample_id, sample_id)
                            ggdf$condition <- factor(md$Group[mm])
                            write.csv(ggdf,paste0(newclustDir,"/boxplot_CLUSTERX_data.csv"))
                            
                            ggsave(filename = paste0(newclustDir,"/boxplot_CLUSTERX.",device),
                                   ggplot(ggdf) +
                                     geom_boxplot(aes(x = condition, y = proportion, color = condition,
                                                      fill = condition), position = position_dodge(), alpha = 0.5,
                                                  outlier.color = NA) +
...
### make group_ids
if(class(fcs)=="flowSet")
{group_ids <- rep(md$Group, fsApply(fcs, nrow))
}else if (class(fcs)=="flowFrame")
{group_ids <- rep(md$Group,nrow(fcs))}
...
## Facet per Group
df<-group_downsample(dr,downsample)
ggp <- ggplot(df,  aes(x = tSNE1, y = tSNE2, color = cell_clust_FS)) +
  geom_point(size = 0.2) +
  theme_bw() +
  scale_color_manual(values = colp) +
  guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))
...
## Facet per Group
df<-group_downsample(dr,downsample)
ggp <- ggplot(df,  aes(x = tSNE1, y = tSNE2, color = cell_clust_CX)) +
  geom_point(size = 0.2) +
  theme_bw() +
  scale_color_manual(values = colp) +
  guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))
