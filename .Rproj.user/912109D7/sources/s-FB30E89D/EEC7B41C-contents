...
--cellSelectionFile=<csvfile>  [defaul cell_select.csv in inputDir] else any filename in inputDir. This file contains the cells selected in a previous run for t-sne
" -> doc
...
tsne_inds <- unlist(tsne_inds)
  ...
  tsne_inds<-read.csv(paste0(inputDir,"/",cellSelectionFile))
}

tsne_expr <- expr[tsne_inds,]
write.csv(x = tsne_inds,file = paste0(inputDir,"/cellSelectionFile.csv"),row.names = F,col.names = F)
